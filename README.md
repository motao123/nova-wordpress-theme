...
### 1.5.5 (2026-04-29)
- 🐛 修复 Markdown 评论图片语法不显示的问题，现在支持 `![图片描述](图片URL)` 渲染为图片。
- 🛡️ 为 Markdown 评论图片输出增加安全白名单，仅允许安全的 `img` 属性，并继续使用 `esc_url` / `wp_kses` 过滤，避免不安全协议或属性注入。
- 🎨 补充评论区 Markdown 图片样式 `.comment-markdown-image`，确保图片自适应评论区域宽度并保持圆角显示。
- 📝 更新自定义器中“启用Markdown评论”的说明，明确支持图片语法。

### 1.5.4 (2026-04-29)
- 🔍 完成项目代码审计，重点检查主题模板、`functions.php`、评论、搜索、AJAX、Markdown解析和前端交互逻辑。
- 🛡️ 强化输出安全：对模板中的标题、链接、属性、描述、评论相关展示等位置统一核查并补充转义处理，降低XSS风险。
- 🔐 强化AJAX安全：阅读量与点赞相关请求使用 nonce 校验、文章ID校验和输入清理，避免未授权或异常请求导致数据污染。
- ⚙️ 新增/完善 Nova 功能开关：文章目录、图片放大、二维码分享、代码复制、代码语言标签、文章点赞、文章过期提示等功能均可在自定义器中控制。
- 📚 完善文章目录 TOC：前端自动生成文章目录，并支持主题开关控制、当前标题高亮和移动端样式优化。
- 🖼️ 完善文章图片点击放大：支持文章图片灯箱预览、关闭交互和移动端适配。
- 🔗 完善二维码分享：支持二维码弹窗分享、复制链接等交互，并增加暗色模式样式。
- 💻 完善代码块体验：支持代码复制按钮、代码语言标签显示，并补充 `.code-language-label` 样式。
- ❤️ 完善文章点赞功能：支持前端点赞交互、点赞状态记录、AJAX更新计数和按钮样式。
- ⏰ 新增文章过期提示：可按设定天数提醒旧文章内容可能过时，并补充 `.stale-post-notice` 明暗色样式。
- 📖 完善阅读辅助信息：展示文章字数、预计阅读时间、阅读进度条和阅读量统计。
- 🧭 完善站点导航与推荐：新增/完善面包屑导航、相关文章推荐、404辅助链接等内容发现能力。
- ✅ 已通过 `php -l functions.php && php -l single.php` 语法检查。
